...
def get_pascalVOC_labels():
    return np.array([(0, 0, 0),  # 0=background
...
def decode_segmap(image, nc=21, dataset='pascalVOC'):
    
    if dataset == "pascalVOC":
        label_colors = get_pascalVOC_labels()
...
def pair_transform_train(image, target, max_cls=34):
    """
    ...
    return image, torch.clamp(target, max=max_cls)
    # return image, target


# -

def pair_transform_val(image, target, max_cls=34):
    """
    Perform appropriate transformations on image and segmentation map.
    For segmap, only convert to tensor and apply augmentation, no normalization etc.
    Args:
